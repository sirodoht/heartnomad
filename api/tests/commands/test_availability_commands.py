...
@unittest.skip(
    "needs fixing after refactoring in ac1c446e4758b2552e5d1bc08c9f832faa31a3a0"
)
class AddCapacityChangeTestCase(TestCase):
    ...
    def test_that_command_with_valid_data_creates_an_capacity(self):
        command = UpdateOrAddCapacityChange(
            self.user, start_date="4016-01-13", resource=self.resource.pk, quantity=2
        )
    ...
    def test_that_command_from_non_house_admin_fails(self):
        ...
        command = UpdateOrAddCapacityChange(
            non_admin, start_date="4016-01-13", resource=self.resource.pk, quantity=2
        )
        self.assertFalse(command.execute())

    def test_that_command_with_missing_data_fails(self):
        command = UpdateOrAddCapacityChange(
            self.user, resource=self.resource.pk, quantity=2
        )

        self.assertFalse(command.execute())

        expected_data = {"errors": {"start_date": ["This field is required."]}}
        self.assertEqual(command.result().serialize(), expected_data)

    def test_that_a_date_in_the_past_fails(self):
        command = UpdateOrAddCapacityChange(
            self.user, start_date="1016-01-13", resource=self.resource.pk, quantity=2
        )
    ...
    def test_that_changing_capacity_to_the_same_quantity_has_no_effect(self):
        ...
        command = UpdateOrAddCapacityChange(
            self.user, start_date="4016-01-15", resource=self.resource.pk, quantity=2
        )
        ...
        expected_data = {
            "warnings": {
                "quantity": ["This is not a change from the previous capacity"]
            }
        }
        self.assertEqual(command.result().serialize(), expected_data)
    ...
    def test_setting_capacity_to_another_quantity_updates_a_record_on_that_date(self):
        ...
        command = UpdateOrAddCapacityChange(
            self.user, start_date="4016-01-13", resource=self.resource.pk, quantity=3
        )
        self.assertTrue(command.execute())
        self.assertEqual(CapacityChange.objects.count(), 1)
    ...
    def test_cannot_udpate_capacity_in_the_past(self):
        ...
        command = UpdateOrAddCapacityChange(
            self.user, start_date="1016-01-13", resource=self.resource.pk, quantity=3
        )
        self.assertFalse(command.execute())
        self.assertEqual(CapacityChange.objects.count(), 1)
    ...
    def test_that_update_command_from_non_house_admin_fails(self):
        ...
        non_admin = UserFactory(username="samwise")
        command = UpdateOrAddCapacityChange(
            non_admin, start_date="4016-01-13", resource=self.resource.pk, quantity=3
        )
        self.assertFalse(command.execute())
