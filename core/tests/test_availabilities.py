...
class CapacityQuantityOnTestCase(TestCase):
    ...
    def test_quantity_is_zero_with_no_capacities(self):
        self.assertEqual(
            CapacityChange.objects.quantity_on(date(4016, 1, 13), self.resource), 0
        )
    ...
    def test_quantity_after_capacity(self):
        self.create_on(date(4016, 1, 14), 3)
        self.assertEqual(
            CapacityChange.objects.quantity_on(date(4016, 1, 14), self.resource), 3
        )
        self.assertEqual(
            CapacityChange.objects.quantity_on(date(4016, 1, 15), self.resource), 3
        )
    ...
    def test_quantity_doesnt_include_other_resource(self):
        ...
        self.create_on(date(4016, 1, 14), 3)
        self.assertEqual(
            CapacityChange.objects.quantity_on(date(4016, 1, 14), other_resource), 0
        )
    ...
    def test_quantity_with_two_capacities(self):
        ...
        self.assertEqual(
            CapacityChange.objects.quantity_on(date(4016, 1, 16), self.resource), 1
        )
        self.assertEqual(
            CapacityChange.objects.quantity_on(date(4016, 1, 17), self.resource), 1
        )
