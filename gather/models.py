...
class EventAdminGroup(models.Model):
    ...
    location = models.OneToOneField(
        Location, related_name="event_admin_group", on_delete=models.CASCADE
    )
...
class EventManager(models.Manager):
    def upcoming(self, upto=None, current_user=None, location=None):
        ...
        upcoming = (
            qs.filter(end__gte=today).exclude(status=Event.CANCELED).order_by("start")
        )
...
class Event(models.Model):
    ...
    organizers = models.ManyToManyField(
        User, related_name="events_organized", blank=True
    )
    ...
    endorsements = models.ManyToManyField(
        User, related_name="events_endorsed", blank=True
    )
    ...
    admin = models.ForeignKey(
        EventAdminGroup, related_name="events", on_delete=models.CASCADE
    )
...
class EventNotifications(models.Model):
    user = models.OneToOneField(
        User, related_name="event_notifications", on_delete=models.CASCADE
    )
    # send reminders on day-of the event?
    reminders = models.BooleanField(default=True)
    # receive weekly announcements about upcoming events?
    location_weekly = models.ManyToManyField(
        Location, related_name="weekly_event_notifications"
    )
