...
class LocationSettingsForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "max_booking_days": forms.TextInput(
                attrs={"class": "form-control", "size": "16"}
            ),
            "welcome_email_days_ahead": forms.TextInput(
                attrs={"class": "form-control", "size": "8"}
            ),
            "house_access_code": forms.TextInput(
                attrs={"class": "form-control", "size": "32"}
            ),
            "ssid": forms.TextInput(attrs={"class": "form-control", "size": "32"}),
            "ssid_password": forms.TextInput(
                attrs={"class": "form-control", "size": "32"}
            ),
            "email_subject_prefix": forms.TextInput(
                attrs={"class": "form-control", "size": "32"}
            ),
...
class LocationContentForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "front_page_stay": forms.Textarea(
                attrs={
                    "class": "form-control",
                    "rows": "16",
                    "cols": "100",
            ...
            "front_page_participate": forms.Textarea(
                attrs={
                    "class": "form-control",
                    "rows": "16",
                    "cols": "100",
...
class LocationPageForm(forms.Form):
    ...
    slug = forms.CharField(
        widget=forms.TextInput(attrs={"class": "form-control", "size": "32"})
    )
...
class BookingUseForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "arrive": forms.DateInput(
                attrs={"class": "datepicker form-control form-group"}
            ),
            "depart": forms.DateInput(
                attrs={"class": "datepicker form-control form-group"}
            ),
    ...
    def __init__(self, location, *args, **kwargs):
        ...
        if self.instance.pk:
            if not hasattr(self.instance, "booking"):
                raise Exception(
                    "This form requires a Booking object to be associated with the Use"
                )
...
class BookingEmailTemplateForm(EmailTemplateForm):
    def __init__(self, tpl, booking, location):
        ...
        self.fields["subject"].initial = (
            ...
            + "] "
            + Template(tpl.subject).render(Context(template_variables))
            + " (#"
            + str(booking.id)
            + ")"
        )
        self.fields["body"].initial = Template(tpl.body).render(
...
class SubscriptionEmailTemplateForm(EmailTemplateForm):
    def __init__(self, tpl, subscription, location):
        ...
        self.fields["footer"].initial = (
            """--------------------------------\nYour membership id is %d. Manage your membership from your profile page https://%s/people/%s."""
            % (subscription.id, domain, subscription.user.username)
        )
